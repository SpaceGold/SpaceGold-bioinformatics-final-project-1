...
```{r genetic-table}
# Create a facet figure showing the distribution of SNPs
locations_facet_data <- vcf_with_metadata %>%
  group_by(pos, ref, alt) %>%
  tally()

locations_facet <- ggplot(data = locations_facet_data,
                          aes(x = pos, y = ref)) +
  geom_point() +
  facet_grid()
...
ggsave(locations_facet,
       filename = "output/figures/locations_facet.png")
```

**Figure 6**: The greatest numbers of high quality, distinct SNPs in the named genes came from Maranhao and Para, Brazil.
