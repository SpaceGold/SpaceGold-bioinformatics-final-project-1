...
bra_epi_tab <- dplyr::filter(.data = epi_tab, countrycode == "BRA", adm_area_1 
                             %in% c("Amazonas", "São Paulo", "Sao Paulo"))
```
...
```{r confirmed-plot}
# produce a dual line plot of confirmed cases by state, Amazonas or Sao Paulo
...
# split into facet grid, or just color by state(yes)

confirmed_plot
```

**Figure 2**: The state of Sao Paulo saw more total Confirmed cases than the state of Amazonas.
