...
**Figure **: The state of Sao Paulo saw more total confirmed cases than the state of Amazonas.
...
population_plot <- population_path %>%
  group_by(population) %>%
  dplyr::filter(RegionName %in% c("Amazonas", "São Paulo", "Total"))

# Remove Amazonas and Sao Paulo populations from Total
population_plot[3,3] <- (population_plot[3,3] - population_plot[2,3] - population_plot[1,3])

# Basic piechart
ggplot(population_plot, aes(x="", y=population, fill=RegionName)) +
...
per_capita_amazonas <- bra_epi_tab %>%
  ...
  group_by(date) %>%
  dplyr::mutate(per_capita = confirmed/4207714) %>%
  dplyr::select(date, per_capita)

per_capita_sp <- bra_epi_tab %>%
  dplyr::filter(adm_area_1 == "São Paulo") %>%
  group_by(date) %>%
  dplyr::mutate(per_capita = confirmed/46289333) %>%
  dplyr::select(date, per_capita)

# per_capita_amazonas
# per_capita_sp
per_capita_plot <- ggplot() +
  geom_line(data=per_capita_amazonas, aes(x = date, y = per_capita))
  geom_line(data=per_capita_sp, aes(x = date, y = per_capita))
...
**Figure**: Amazonas saw greater confirmed cases per capita than Sao Paulo.
...
prevalence_plot <- prevalence_path %>%
  ...
  labs(title = "Seroreversion adjusted prevalence of SARS-CoV-2 antibodies
       ...
       x = "Month",
       y = "Manaus seroreversion adjusted prevalence")
...
**Figure**: Manaus saw more antibody prevalence of SARS-CoV-2 than Sao Paulo.
